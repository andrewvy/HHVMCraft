...
class NBT {
  ...
  public function loadFile($filename, $wrapper = "compress.zlib://") {
    if (is_string($wrapper) && is_file($filename)) {
      if ($this->verbose) trigger_error("Loading file \"{$filename}\" with stream wrapper \"{$wrapper}\".",
        E_USER_NOTICE);
  ...
  public function readType($fp, $tagType) {
    switch ($tagType) {
      ...
      case self::TAG_FLOAT: // Floating point value (32 bit, big endian, IEEE 754-2008)
        list(, $value) = (pack('d', 1) == "\77\360\0\0\0\0\0\0") ? unpack('f', fread($fp, 4)) : unpack('f',
          strrev(fread($fp, 4)));

        return $value;
      case self::TAG_DOUBLE: // Double value (64 bit, big endian, IEEE 754-2008)
        list(, $value) = (pack('d', 1) == "\77\360\0\0\0\0\0\0") ? unpack('d', fread($fp, 8)) : unpack('d',
          strrev(fread($fp, 8)));
      ...
      case self::TAG_STRING: // String
        ...
        $string = utf8_decode(fread($fp,
          $stringLength)); // Read in number of bytes specified by string length, and decode from utf8.
        return $string;
  ...
  public function writeFile($filename, $wrapper = "compress.zlib://") {
    if (is_string($wrapper)) {
      if ($this->verbose) trigger_error("Writing file \"{$filename}\" with stream wrapper \"{$wrapper}\".",
        E_USER_NOTICE);
    ...
    if ($this->verbose) trigger_error("Writing " . count($this->root) . " root tag(s) to file/resource.",
      E_USER_NOTICE);
    foreach ($this->root as $rootNum => $rootTag) if (!$this->writeTag($fp, $rootTag)
  ...
  public function writeTag($fp, $tag) {
    ...
    return $this->writeType($fp, self::TAG_BYTE, $tag["type"]) && $this->writeType($fp, self::TAG_STRING,
      $tag["name"]) && $this->writeType($fp, $tag["type"], $tag["value"]);
  }
  ...
  public function writeType($fp, $tagType, $value) {
    switch ($tagType) {
      ...
      case self::TAG_LONG: // Signed long (64 bit, big endian)
        ...
        return is_int(fwrite($fp, pack("N", gmp_intval($firstHalf)))) && is_int(fwrite($fp,
          pack("N", gmp_intval($secondHalf))));
      case self::TAG_FLOAT: // Floating point value (32 bit, big endian, IEEE 754-2008)
        return is_int(fwrite($fp,
          (pack('d', 1) == "\77\360\0\0\0\0\0\0") ? pack('f', $value) : strrev(pack('f', $value))));
      case self::TAG_DOUBLE: // Double value (64 bit, big endian, IEEE 754-2008)
        return is_int(fwrite($fp,
      ...
      case self::TAG_LIST: // List
        if ($this->verbose) trigger_error("Writing list of " . count($value["value"]) . " tags of type {$value["type"]}.",
          E_USER_NOTICE);
        if (!($this->writeType($fp, self::TAG_BYTE, $value["type"]) && $this->writeType($fp, self::TAG_INT,
            count($value["value"])))
        ) return false;
